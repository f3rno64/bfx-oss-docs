 ...
 * @param {string} [args.json] - if specified, output will be JSON data with
 *   this filename
 ...
 */
const getUnifiedConfig = ({ json, output, includes }) => {
  const isJSON = _isString(json) && !_isEmpty(json)
  ...
  return {
    ...
    opts: {
      destination: isJSON ? `${output}/${json}` : output,
      encoding: 'utf8',
      private: false,
      recurse: true,
      verbose: true,
      template: `./node_modules/${isJSON ? 'jsdoc-json' : 'docdash'}`,
