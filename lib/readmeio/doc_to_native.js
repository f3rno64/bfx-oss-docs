...
const processRIOSidebarCodeSamples = require('./process_sidebar_code_samples')
const rioBlockToMarkdown = require('./block_to_md')
...
const rioDocumentToNative = (data) => {
  const { title, slug, id, description, elements } = data
  const nativeData = {
    id: id.replace(/-/g, '_'),
    files: [],
    description,
  ...
  if (!_isEmpty(description)) {
    mdLines.push(`subtitle: "${description.replace(/"/g, '\\"')}"`)
  }
